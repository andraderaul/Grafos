...
void profundidade(vector <int> P, string S){
	output << endl;
	output << endl << "[" << S << "]" << endl;
	for(size_t i = 0; i < P.size(); i++)
		output <<"Vertice: " << P[i] << " - Profundidade "<< i+1 << endl;
}
void DFS(tEAD* G, int v){
	...
	pIN.push_back(v); //vetor de profundidade entrada
	...
	//	cout << "Visitado: "<< w << endl;
		...
		for(size_t i = 0; i < G[w].adj.size(); i++){ // para todas as arestas (w,u)
			...
			if(!G[u].marcado){ // se u é não marcado
				...
				pIN.push_back(u); // vetor de profundidade 
				break;
			}			
		}
		if(G[u].marcado){
			pOU.push_back(visitado.top()); //vetor de profundidade saida
			visitado.pop();
		}
		
	}
	
	profundidade(pIN,"Entrada");
	profundidade(pOU,"Saida");
}
